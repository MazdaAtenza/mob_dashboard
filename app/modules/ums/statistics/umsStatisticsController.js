componentApp.controller("umsStatisticsController", ["$scope", "umsStatisticsList", "umsSaveDate", function($scope, umsStatisticsList, umsSaveDate){
     ...
     * 输入每页显示的信息数
     */
    $scope.changeNum = function(tableContent, page_size, platformNo){
        page_size = Number(page_size);
        if(page_size <= 0 || page_size == undefined || isNaN(page_size)){
            return
        }

        tableContent[platformNo].page_size = page_size;
        tableContent[platformNo].cur_page = 1;
       

        tableContent[platformNo].cur_shown_list = [];
        if(tableContent[platformNo].statisticsList.length <= page_size){
            for(var i=0; i<tableContent[platformNo].statisticsList.length; i++){
                tableContent[platformNo].cur_shown_list[i] = tableContent[platformNo].statisticsList[i];
            }
        }else{
            for(var j=0; j<page_size; j++){
                tableContent[platformNo].cur_shown_list[j] = tableContent[platformNo].statisticsList[j];
            }
        }
        initCur_shown_list(tableContent, tableContent[platformNo].tabNo);
    }
